...
When(/^I select the "([^"]*)" bond$/) do |bond|
  bond.gsub!(' ','')
  @current_page.select_bond(bond)
end

When(/^I select amount of "([^"]*)"$/) do |amount|
  amount.gsub!('$','')
  @current_page.select_amount(amount)
end

When(/^I enter "([^"]*)"$/) do |enter|
  enter.gsub!(' ','')
  @current_page.enter_date(enter)
end

When(/^I click "([^"]*)"$/) do |click|
  click.upcase!
  @current_page.click_button(click)
end
